...
personal_tv_shows_full_data <- house_of_cards_full_data %>% 
  bind_rows(ozark_full_data, breaking_bad_full_data, better_call_saul_full_data, suits_full_data, stranger_things_full_data)

personal_tv_shows_full_data %>%
  ggplot(aes(x = words_per_minute, y = fct_reorder(tv_series, words_per_minute, median))) +
  geom_boxplot(aes(fill = tv_series), outlier.shape = NA, width = 0.25, alpha = 0.5) +
  geom_jitter(aes(color = tv_series), size = 5, alpha = 0.25, shape = 19, 
  ...
  annotate(
    geom = "text",
    x = c(145, 140, 67, 161, 82, 80, 95), 
    y = c(3.52, 4.5, 5.05, 5.1, 6.2, 6.05, 5.9),
    label = c("S3.E5 - Chicanery", "S1.E8 - Kaleidoscope", "S3.E13 - Chapter 39", "S1.E9 - Chapter 9", "S1.E3 - Inside Track", "S1.E2 - Errors and Omissions", "S1.E1 - Pilot"),
    fontface = 2
  ) + 
